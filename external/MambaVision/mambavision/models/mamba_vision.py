...
try:
    ...
    def selective_scan_fn(u, delta, A, B, C, D, z=None, delta_bias=None, delta_softplus=True, return_last_state=None):
        ...
        for idx in range(seqlen):
            ...
            dB_u = dt.unsqueeze(-1) * B_t.unsqueeze(1) * u_t.unsqueeze(-1)
            state = state * dA + dB_u
            y = (state * C_t.unsqueeze(1)).sum(-1)
            y = y + D.float().unsqueeze(0) * u_t
            outputs.append(y.to(dtype=u.dtype))
